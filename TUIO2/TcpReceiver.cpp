...
#endif
{
	...
	std::cout << "closed " << type_type() << " connection" << std::endl;
...
#endif
{
	...
	while ((int)sender->tcp_socket>0) {
		...
		if (tcp_client>0) { 
			std::cout << "listening to " << type_type() << " messages from " << inet_ntoa(client_addr.sin_addr) << "@" << client_addr.sin_port << std::endl;
...
, locked	(false)
{
	...
	if (tcp_socket < 0) std::cerr << "could not create " << type_type() << " socket" << std::endl;
...
#endif
	if (ret < 0) {
		std::cerr << "could not reuse " << type_type() << " socket address" << std::endl;
		return;
	}
	...
	if (ret < 0) {
		std::cerr << "could not bind to " << type_type() << " socket on port " << port << std::endl;
		return;
	}
	
	ret =  listen(tcp_socket, 1);
	if (ret < 0) {
		std::cerr << "could not start listening to " << type_type() << " socket" << std::endl;
#ifdef WIN32
	...
	std::cout << type_type() << " socket created on port " << port << std::endl;
}
...
, locked		(false)
{
	...
	if (tcp_socket < 0) {
		std::cerr << "could not create " << type_type() << " socket" << std::endl;
		return;
	}
	...
	if (ret<0) {
		...
		close(tcp_socket);
#endif		
		std::cerr << "could not connect to " << type_type() << " server at " << host << ":"<< port << std::endl;
		tcp_socket=-1;
		return;
	} else {
		tcp_client_list.push_back(tcp_socket);
		std::cout << "listening to " << type_type() << " messages from " << host << ":" << port << std::endl;
	}
}
